...
| Vite 8 (pnpm override)                    | Tracked in [#381](https://github.com/oaslananka/kicad-studio-kit/issues/381). Still blocked by `vitepress` 1.6.4 (the newest stable VitePress; only `2.0.0-alpha` supports Vite 8). The `vite` override drives the VitePress docs build, and Vite 8 (Rolldown bundler, removed `transformWithEsbuild`) fails `vitepress build docs` against VitePress 1.6.4. Renovate Dashboard item [#244](https://github.com/oaslananka/kicad-studio-kit/issues/244) remains pending approval until a stable VitePress release supports Vite 8. |
...
### Vite 8 upgrade plan (blocked)

- Keep Vite 8 isolated to a dedicated dependency PR after `vitepress` publishes stable Vite 8 compatibility.
- Review Vite 8 and VitePress migration notes before raising the PR.
- Keep the code changes minimal: `pnpm-workspace.yaml` override, lockfile refresh, and only compatibility fixes required by failing checks.
- Required validation once unblocked:
  ...
  - `pnpm run build`
  - `pnpm run check`
  - Full CI matrix on all platforms
- Rollback path: revert the Vite 8 override/lockfile bump to the last Vite 6.x state and rerun the same validation matrix.
