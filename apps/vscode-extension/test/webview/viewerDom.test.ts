...
test.describe('KiCad Studio webview DOM', () => {
  ...
  test('preserves the CLI fallback diagnostic when fallback rendering throws', async ({
    page
  }) => {
    await installVsCodeApiMock(page, '<div>not-an-svg</div>');
    await setViewerContent(
      page,
      {
        fileName: 'invalid-fallback.kicad_pcb',
        fileType: 'board',
        base64: readFixtureBase64('sample.kicad_pcb')
      },
      { surface: 'none', failure: 'webgl-init' }
    ...
    await expect(page.locator('#error-detail')).toContainText(
      'CLI SVG fallback also failed: The fallback SVG could not be normalized'
    );
    await expect.poll(() => countMessages(page, 'requestSvgFallback')).toBe(1);
  });
  ...
  test('attempts CLI SVG fallback only once when a non-drawable renderer cannot recover', async ({
    page
  }) => {
    await installVsCodeApiMock(page);
    await setViewerContent(
      page,
      {
        fileName: 'non-drawable-failure.kicad_pcb',
        fileType: 'board',
        base64: readFixtureBase64('sample.kicad_pcb')
      },
      { surface: 'none' }
