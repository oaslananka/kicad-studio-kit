...
The MCP server (`kicad-mcp-pro`) source and version files now live in [oaslananka/kicad-mcp-pro](https://github.com/oaslananka/kicad-mcp-pro).
...
- Release Please explicitly dispatches `publish-extension.yml` after creating a
  ...
  Release.
- `publish-python.yml` (now in [oaslananka/kicad-mcp-pro](https://github.com/oaslananka/kicad-mcp-pro)) validates the wheel and source distribution, emits SHA256SUMS.txt, emits a CycloneDX SBOM,
...
- `publish-mcp-container.yml` (now in [oaslananka/kicad-mcp-pro](https://github.com/oaslananka/kicad-mcp-pro)) validates the Docker image on pull requests and
...
- `kicad-mcp-pro` for `oaslananka/kicad-mcp-pro` (source in separate repository).
