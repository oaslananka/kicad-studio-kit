...
The production graph contains 148 TypeScript modules. After the CLI capability-model, viewer-controller, export-command-builder, Component Search view, PCM catalog, PCM archive, and PCM installed-state persistence extractions, the graph contains **0 import cycles**. The repository enforces this with `pnpm run check:vscode-architecture`.
...
|     5 | `library/pcmService.ts` / `library/pcmCatalog.ts` / `library/pcmArchive.ts` / `library/pcmPersistence.ts` | 730 / 315 / 641 / 110 | 5 / new / new / new | install orchestration plus catalog, archive, and installed-state adapters           | **Completed through phase 5c:** installed state and KiCad JSON persistence extracted; library-table adapter remains |
...
`library/pcmService.ts` continues to own repository fetching, install-path selection, injected extractor support, CLI-backed installation, in-memory package state, KiCad library-table persistence, reindexing, configuration lookup, and user-visible change events. Phase 5c reduces the service from 798 to 730 lines without changing install, update, or uninstall behavior. KiCad library-table persistence remains the next separate #497 phase with dedicated S-expression and path regression gates.
