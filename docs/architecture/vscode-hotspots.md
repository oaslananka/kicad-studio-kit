...
|     5 | `library/pcmService.ts` / `library/pcmCatalog.ts` / `library/pcmArchive.ts`          | 798 / 315 / 641 |  4 / new / new | VS Code install orchestration plus extracted catalog and hardened archive adapters  | **Completed through phase 5b and hardened in #563:** bounded ZIP validation/staging; persistence/table adapters remain |
...
`library/pcmService.ts` continues to own repository fetching, install-path selection, injected extractor support, CLI-backed installation, extension state, KiCad library-table persistence, reindexing, configuration lookup, and user-visible change events. Phase 5b reduced the service from 891 to 790 lines; issue #563 adds the staged-install boundary and brings it to 798 lines while preserving direct-install behavior. Persistence and library-table adapters remain separate #497 phases with migration and state regression gates.
