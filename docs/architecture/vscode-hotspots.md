...
The production graph contains 147 TypeScript modules. After the CLI capability-model, viewer-controller, export-command-builder, Component Search view, PCM catalog, and PCM archive extractions, the graph contains **0 import cycles**. The repository enforces this with `pnpm run check:vscode-architecture`.
...
|     5 | `library/pcmService.ts` / `library/pcmCatalog.ts` / `library/pcmArchive.ts`          | 790 / 315 / 100 |  3 / new / new | VS Code install orchestration plus extracted catalog and archive adapters           | **Completed through phase 5b:** pure catalog model and reviewed checksum/ZIP adapter; persistence/table adapters remain |
...
`components/componentSearch.ts` remains the VS Code-facing orchestration surface. It owns provider selection, request and cache fallbacks, local-library and PCM integration, settings and secrets, project-context recommendation discovery, view-model calculation, panel lifecycle, and command handling. It preserves the previous renderer imports through compatibility re-exports.

Phase 4a preserves representative generated search and details HTML byte-for-byte while reducing `componentSearch.ts` from 1,215 to 784 lines. Provider-adapter and ranking-model extraction remain separate #497 phases with their own network/cache and relevance regression gates.
...
`library/pcmArchive.ts` owns SHA-256 verification and the reviewed ZIP extraction adapter. Its dependency allowlist is limited to `node:crypto`, `node:fs`, `node:path`, and `node:zlib`; it cannot import VS Code or another repository module. Direct tests cover stored and deflate entries, directory entries, malformed archives, unsupported compression, checksum mismatch context, and traversal-name rejection.

`library/pcmService.ts` continues to own repository fetching, install-path selection, injected extractor support, CLI-backed installation, extension state, KiCad library-table persistence, reindexing, configuration lookup, and user-visible change events. Phase 5b reduces the service from 891 to 790 lines while preserving existing direct-install behavior. Persistence and library-table adapters remain separate #497 phases with migration and state regression gates.
