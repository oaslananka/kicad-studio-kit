...
The production graph contains 146 TypeScript modules. After the CLI capability-model, viewer-controller, export-command-builder, and Component Search view extractions, the graph contains **0 import cycles**. The repository enforces this with `pnpm run check:vscode-architecture`.
...
|     5 | `library/pcmService.ts` / `library/pcmCatalog.ts`                                    |      891 / 317 |        3 / new | VS Code install orchestration plus the extracted immutable catalog model            | **Completed in phase 5a:** pure catalog types/normalization/versioning/search; archive installer and persistence adapters remain |
...
`library/pcmService.ts` preserves the public import surface through compatibility re-exports and continues to own repository fetching, checksum verification, ZIP extraction, install paths, CLI-backed installation, extension state, KiCad library-table persistence, reindexing, configuration lookup, and user-visible change events. Phase 5a reduces `pcmService.ts` from 1,142 to 891 lines without changing package behavior. Archive verification/installation and persistence/table adapters remain separate #497 phases with dedicated security and migration regression gates.
