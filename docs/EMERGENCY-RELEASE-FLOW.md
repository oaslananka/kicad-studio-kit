...
A broken schema published from `oaslananka/kicad-mcp-pro`. Consumers detect the
break when:
...
**Root cause**: `oaslananka/kicad-mcp-pro` shipped with a logic regression,
incompatible `compatibility.yaml` range, or protocol schema drift.

### A.3 Bad npm kicad-mcp-pro launcher

A broken npm launcher published from `oaslananka/kicad-mcp-pro`. Detection:
...
| **Publish fix**                | Push a patch version from `oaslananka/kicad-mcp-pro`, let the publish workflow run                                                                |
...
| **Publish fix**               | Bump version in `oaslananka/kicad-mcp-pro`, let Release Please + `publish-python.yml` handle it |
...
| Delete packages/mcp-server             | Completed — source moved to `oaslananka/kicad-mcp-pro` |
...
4. `packages/mcp-server` has been removed — the source now lives in `oaslananka/kicad-mcp-pro`.
